...
def delete_user_admin(user_id: int, current_user: models.User = Depends(get_current_admin_user), db: Session = Depends(get_db)):
    ...
    # 1. Usuń członkostwo w rodzinie
    db.query(models.FamilyMember).filter(models.FamilyMember.user_id == user_id).delete()

    # 2. Usuń zaproszenia
    db.query(models.FamilyInvitation).filter(
        models.FamilyInvitation.invited_username == target_user.username
    ).delete()

    # 3. Jeśli użytkownik jest założycielem rodziny – usuń całą rodzinę
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, host="0.0.0.0", port=port, ws="websockets")  # ← DODAJ ws="websockets"